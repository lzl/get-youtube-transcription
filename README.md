...
1. **字幕接口优先**：解析 YouTube 页面中的 `ytInitialPlayerResponse` / `ytInitialData`，优先请求 YouTube 自身的字幕接口
2. **页面面板回退**：如果字幕接口不可用，则展开并读取 YouTube 原生 transcript 面板
...
- 仅请求 `https://www.youtube.com/*` 主机权限
- 响应式设计，适配各种屏幕尺寸
- 兼容 YouTube 的明暗主题
- 智能检测页面变化，支持 SPA 导航
